"""
Lambda handlers for SageMaker Studio package installer lifecycle configuration.

This module provides the Lambda functions required to implement a custom resource
for managing package installation lifecycle configurations in SageMaker Studio.
"""
...
INSTALL_SCRIPT_FILENAME = "install-packages.sh"


def read_script_content() -> str:
    """
    Read the package installation script from file.

    Returns:
        Base64-encoded content of the installation script.

    Raises:
        FileNotFoundError: If the script file cannot be found.
    """
    script_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), INSTALL_SCRIPT_FILENAME
    )
    logger.info("Reading installation script", extra={"script_path": script_path})
    ...
    try:
        ...
        logger.error(
            "Installation script not found", extra={"script_path": script_path}
        )
...
def create_lifecycle_config(domain_id: str, config_name: str) -> Dict[str, Any]:
    """
    Create a new lifecycle configuration for package installation.
    ...
    """
    logger.info(
        "Creating package installer lifecycle configuration",
...
def update_lifecycle_config(
    ...
    """
    logger.info(
        "Updating package installer lifecycle configuration",
        extra={"domain_id": domain_id, "config_name": config_name},
    )
...
def delete_lifecycle_config(
    ...
    """
    logger.info(
        "Deleting package installer lifecycle configuration",
...
def is_deletion_complete(
    config_name: str, event: Dict[str, Any], context: LambdaContext
) -> Dict[str, bool]:
    """
...
def is_complete_handler(
    ...
    if request_type == "Create":
        return is_creation_complete(event, context)

    if request_type == "Update":
        return is_update_complete(event, context)

    if request_type == "Delete":
        return is_deletion_complete(config_name, event, context)
